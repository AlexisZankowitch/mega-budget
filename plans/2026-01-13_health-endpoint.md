...
## Steps
1. Add `cmd/spendtrack/main.go` with Fx app wiring.
...
## Verification
- `go test ./...`
- `go run ./cmd/spendtrack` with `DATABASE_URL` or `DEV_DATABASE_URL` set, then `curl -i localhost:8080/healthz`.

## Rollback
- Remove `cmd/spendtrack` and `internal` packages.
